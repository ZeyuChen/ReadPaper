...
@patch("app.backend.arxiv_translator.main.asyncio")
def test_e2e_pipeline_mocked(
    mock_asyncio, mock_analyzer_cls,
    mock_clean, mock_compile_with_fix_loop,
    mock_translator_cls,
    mock_extract, mock_download, mock_workspace
):
    """
    Tests the simplified main() pipeline with mocked I/O.
    New architecture: one file = one Gemini API call, asyncio.gather.
    """
    # ── Mocks Setup ──────────────────────────────────────────────────────────
    ...
    # Translator mock — translate_file returns translated content
    translator_instance = mock_translator_cls.return_value
    translator_instance.translate_file = AsyncMock(return_value=(
        r"\documentclass{article}\begin{document}翻译后内容\end{document}", 100, 80
    ))

    # compile_with_fix_loop — create a dummy PDF
    def side_effect_compile(source_dir, main_tex, **kwargs):
        pdf_path = os.path.join(source_dir, os.path.basename(main_tex).replace(".tex", ".pdf"))
        os.makedirs(source_dir, exist_ok=True)
        with open(pdf_path, "w") as f:
            f.write("%PDF-1.4 dummy")
        return (True, "")
    mock_compile_with_fix_loop.side_effect = side_effect_compile
    ...
    mock_asyncio.Semaphore = real_asyncio.Semaphore
    ...
    assert mock_compile_with_fix_loop.call_count == 1

    final_pdf_path = os.path.join(mock_workspace, "final.pdf")
    assert os.path.exists(final_pdf_path), "Final output PDF should exist"
